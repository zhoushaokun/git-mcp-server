 ...
 */
export function initializeGitStashStateAccessors(getWdFn: GetWorkingDirectoryFn, getSidFn: GetSessionIdFn): void {
 ...
 */
export const registerGitStashTool = async (server: McpServer): Promise<void> => {
      ...
      async (validatedArgs, callContext): Promise<CallToolResult> => {
        ...
        return await ErrorHandler.tryCatch<CallToolResult>(
          async () => {
            ...
            const stashResult: GitStashResult = await gitStashLogic(toolInput, logicContext);
