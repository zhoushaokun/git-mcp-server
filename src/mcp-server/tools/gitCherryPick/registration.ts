 ...
 */
export function initializeGitCherryPickStateAccessors(getWdFn: GetWorkingDirectoryFn, getSidFn: GetSessionIdFn): void {
 ...
 */
export const registerGitCherryPickTool = async (server: McpServer): Promise<void> => {
    ...
    server.tool<typeof GitCherryPickInputSchema.shape>(
      ...
      async (validatedArgs, callContext): Promise<CallToolResult> => {
        ...
        return await ErrorHandler.tryCatch<CallToolResult>(
          async () => {
            const cherryPickResult: GitCherryPickResult = await gitCherryPickLogic(toolInput, logicContext);
