 ...
 */
export function initializeGitCheckoutStateAccessors(getWdFn: GetWorkingDirectoryFn, getSidFn: GetSessionIdFn): void {
 ...
 */
export async function registerGitCheckoutTool(server: McpServer): Promise<void> {
      ...
      async (validatedArgs, callContext): Promise<CallToolResult> => {
        ...
        return await ErrorHandler.tryCatch<CallToolResult>(
          async () => {
            ...
            const checkoutResult: GitCheckoutResult = await checkoutGit(validatedArgs as GitCheckoutInput, logicContext);
