 ...
 */
export function initializeGitRemoteStateAccessors(getWdFn: GetWorkingDirectoryFn, getSidFn: GetSessionIdFn): void {
 ...
 */
export const registerGitRemoteTool = async (server: McpServer): Promise<void> => {
      ...
      async (validatedArgs, callContext): Promise<CallToolResult> => {
        ...
        return await ErrorHandler.tryCatch<CallToolResult>(
          async () => {
            ...
            const remoteResult: GitRemoteResult = await gitRemoteLogic(validatedArgs as GitRemoteInput, logicContext);
