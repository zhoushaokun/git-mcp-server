 ...
 */
export const registerGitCloneTool = async (server: McpServer): Promise<void> => {
      ...
      async (validatedArgs, callContext): Promise<CallToolResult> => {
        ...
        return await ErrorHandler.tryCatch<CallToolResult>(
          async () => {
            ...
            const cloneResult: GitCloneResult = await gitCloneLogic(validatedArgs as GitCloneInput, requestContext);
