 ...
 */
export function initializeGitBranchStateAccessors(getWdFn: GetWorkingDirectoryFn, getSidFn: GetSessionIdFn): void {
 ...
 */
export const registerGitBranchTool = async (server: McpServer): Promise<void> => {
    ...
    server.tool<typeof GitBranchBaseSchema.shape>(
      ...
      async (validatedArgs, callContext): Promise<CallToolResult> => {
        ...
        return await ErrorHandler.tryCatch<CallToolResult>(
          async () => {
            const branchResult: GitBranchResult = await gitBranchLogic(toolInput, logicContext);
