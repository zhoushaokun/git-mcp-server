...
export type GetSessionIdFn = (context: Record<string, any>) => string | undefined; // Re-using from set tool
 ...
 */
export function initializeGitClearWorkingDirStateAccessors(clearFn: ClearWorkingDirectoryFn, getFn: GetSessionIdFn): void {
 ...
 */
export async function registerGitClearWorkingDirTool(server: McpServer): Promise<void> {
      ...
      async (validatedArgs, callContext) => {
        ...
        return await ErrorHandler.tryCatch<CallToolResult>(
          async () => {
            ...
            const result = await gitClearWorkingDirLogic(validatedArgs as GitClearWorkingDirInput, logicContext);
