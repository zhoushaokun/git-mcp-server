...
import { GitWorktreeBaseSchema, GitWorktreeInput, gitWorktreeLogic, GitWorktreeResult } from './logic.js';
 ...
 */
export function initializeGitWorktreeStateAccessors(getWdFn: GetWorkingDirectoryFn, getSidFn: GetSessionIdFn): void {
 ...
 */
export const registerGitWorktreeTool = async (server: McpServer): Promise<void> => {
    ...
    server.tool<typeof GitWorktreeBaseSchema.shape>(
      ...
      async (validatedArgs, callContext): Promise<CallToolResult> => {
        ...
        return await ErrorHandler.tryCatch<CallToolResult>(
          async () => {
            const worktreeResult: GitWorktreeResult = await gitWorktreeLogic(toolInput, logicContext);
