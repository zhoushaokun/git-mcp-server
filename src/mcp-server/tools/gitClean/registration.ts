 ...
 */
export function initializeGitCleanStateAccessors(getWdFn: GetWorkingDirectoryFn, getSidFn: GetSessionIdFn): void {
 ...
 */
export const registerGitCleanTool = async (server: McpServer): Promise<void> => {
      ...
      async (validatedArgs, callContext): Promise<CallToolResult> => {
        ...
        return await ErrorHandler.tryCatch<CallToolResult>(
          async () => {
            ...
            const cleanResult: GitCleanResult = await gitCleanLogic(toolInput, logicContext);
