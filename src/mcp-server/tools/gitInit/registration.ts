...
let _getWorkingDirectory: (sessionId: string | undefined) => string | undefined = () => undefined;
let _getSessionIdFromContext: (context: Record<string, any>) => string | undefined = () => undefined;
...
export const registerGitInitTool = async (server: McpServer): Promise<void> => {
  ...
  await ErrorHandler.tryCatch(async () => {
    server.tool<typeof RegistrationSchema>(
      ...
      TOOL_DESCRIPTION,
      RegistrationSchema,
      async (validatedArgs, callContext): Promise<CallToolResult> => { // Removed explicit type for callContext
        ...
        return await ErrorHandler.tryCatch<CallToolResult>(
          async () => {
            ...
            const initResult: GitInitResult = await gitInitLogic(logicArgs, requestContext);
