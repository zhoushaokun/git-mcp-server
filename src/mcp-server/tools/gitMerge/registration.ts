 ...
 */
export function initializeGitMergeStateAccessors(getWdFn: GetWorkingDirectoryFn, getSidFn: GetSessionIdFn): void {
 ...
 */
export const registerGitMergeTool = async (server: McpServer): Promise<void> => {
      ...
      async (validatedArgs, callContext): Promise<CallToolResult> => {
        ...
        return await ErrorHandler.tryCatch<CallToolResult>(
          async () => {
            ...
            const mergeResult: GitMergeResult = await gitMergeLogic(validatedArgs as GitMergeInput, logicContext);
